...
export default function CustomerDashboard() {
  ...
  const [negotiationInputId, setNegotiationInputId] = useState<string | null>(null);
  const [negotiationText, setNegotiationText] = useState('');

  // Loaded/Sync Notifications list
  useEffect(() => {
    const savedNotifs = localStorage.getItem('vietagri_notifications_customer');
    if (savedNotifs) {
      try {
        setNotificationsList(JSON.parse(savedNotifs));
      } catch (e) {
        console.error(e);
      }
    } else {
      const defaultNotifs = [
        {
          id: 'notif-1',
          type: 'negotiation',
          title: 'Đề xuất Thương thảo từ Super Admin',
          message: 'Yêu cầu điều chỉnh đơn giá Cà phê Arabica Cầu Đất từ 85.000 đ/kg lên 90.000 đ/kg do mùa vụ khan hiếm và kiểm định chất lượng đạt loại A+ nâng cao.',
          timestamp: new Date(Date.now() - 3600000).toISOString(),
          isRead: false,
          contractId: 'contract_01', 
          contractNo: 'HD-2026-CA01',
          meta: {
            negotiationNotes: 'Cà phê đạt chuẩn ẩm xuất khẩu < 12.5%, hạt sàng 18 đều tăm tắp. Đề xuất biểu giá mới 90.000 đ/kg.',
            actioned: false,
            proposedPrice: 90000,
            actionResult: null,
          }
        },
        {
          id: 'notif-2',
          type: 'rejection',
          title: 'Hợp đồng bị Từ chối bởi Super Admin',
          message: 'Yêu cầu cung cấp nấm đùi gà khép kín (trong HĐ số HD-2026-NAM02) bị Từ chối duyệt do sản lượng đề xuất (150 tấn/tháng) vượt quá năng lực đáp ứng hiện tại của Hợp tác xã Ba Vì.',
          timestamp: new Date(Date.now() - 14400000).toISOString(),
          isRead: false,
          contractId: 'contract_03',
          contractNo: 'HD-2026-NAM02',
          meta: {
            rejectReason: 'Sản lượng đề xuất vượt quá công suất thiết kế vùng trồng Lâm Đồng giai đoạn 1.',
            actioned: false,
            actionResult: null,
          }
        },
        {
          id: 'notif-3',
          type: 'approval',
          title: 'Hợp đồng sẵn sàng ký số CA',
          message: 'Hợp đồng liên kết Súp lơ xanh hữu cơ đã được Hợp tác xã ký số đóng mộc đỏ và Super Admin phê duyệt tối cao. Vui lòng ký số điện tử của đại diện Doanh nghiệp để kích hoạt hiệu lực pháp lý.',
          timestamp: new Date(Date.now() - 86400000).toISOString(),
          isRead: false,
          contractId: 'contract_02',
          contractNo: 'HD-2026-SLX03',
          meta: {
            actioned: false,
            actionResult: null,
          }
        },
        {
          id: 'notif-4',
          type: 'success',
          title: 'Thanh toán đợt 1 thành công',
          message: 'Super Admin cùng hệ thống ngân hàng liên kết xác nhận đã nhận thành công khoản thanh toán đợt 1 (trị giá 195.000.000đ) cho đơn hàng Bơ bơ sáp Booth xuất khẩu.',
          timestamp: new Date(Date.now() - 172800000).toISOString(),
          isRead: true,
          contractId: 'contract_04',
          contractNo: 'HD-2026-BOOTH04',
          meta: {
            actioned: true,
      ...
      localStorage.setItem('vietagri_notifications_customer', JSON.stringify(defaultNotifs));
    }
  }, []);

  const markAsRead = (id: string) => {
    const updated = notificationsList.map(n => n.id === id ? { ...n, isRead: true } : n);
    setNotificationsList(updated);
    localStorage.setItem('vietagri_notifications_customer', JSON.stringify(updated));
  };
  ...
  const handleAcceptNegotiation = async (notifId: string, contractNumber: string, proposedPrice?: number) => {
    try {
      const foundContract = contractsList.find(c => c.contractNo === contractNumber || c.id === contractNumber);
      if (foundContract) {
        const updatedContract = { 
          ...foundContract, 
          status: 'signed',
          unitPrice: proposedPrice ? `${proposedPrice.toLocaleString('vi-VN')} đ/kg` : foundContract.unitPrice,
          totalVal: proposedPrice ? (foundContract.totalVolume * proposedPrice) : foundContract.totalVal
        };
        await contractService.updateContract(foundContract.id, updatedContract);
        const updatedContracts = contractsList.map(c => c.id === foundContract.id ? updatedContract : c);
        setContractsList(updatedContracts);
        localStorage.setItem('vietagri_contracts_v2', JSON.stringify(updatedContracts));
      }

      const updatedNotifs = notificationsList.map(n => {
        if (n.id === notifId) {
          return {
            ...n,
            isRead: true,
      ...
      localStorage.setItem('vietagri_notifications_customer', JSON.stringify(updatedNotifs));
      setSuccessBanner(`Hợp đồng ${contractNumber} đã được cập nhật đơn giá đề xuất thành công và chuyển sang trạng thái "Sẵn sàng ký số"!`);
    } catch (e) {
      console.error(e);
    }
  };

  const handleRejectNegotiation = (notifId: string, contractNumber: string) => {
    const updatedNotifs = notificationsList.map(n => {
      if (n.id === notifId) {
        return {
          ...n,
    ...
    setNotificationsList(updatedNotifs);
    localStorage.setItem('vietagri_notifications_customer', JSON.stringify(updatedNotifs));
    setSuccessBanner(`Đã gửi phản hồi từ chối thương thảo đối với hợp đồng ${contractNumber} tới Super Admin.`);
  };

  const handleSendFeedbackNegotiation = async (notifId: string, contractNumber: string, feedbackMsg: string) => {
    ...
    try {
      ...
      if (foundContract) {
        const updatedContract = { 
          ...foundContract, 
          status: 'Thương thảo',
          negotiationNotes: `Doanh nghiệp phản hồi: "${feedbackMsg}"`
        };
        await contractService.updateContract(foundContract.id, updatedContract);
        const updatedContracts = contractsList.map(c => c.id === foundContract.id ? updatedContract : c);
        setContractsList(updatedContracts);
      ...
      const updatedNotifs = notificationsList.map(n => {
        if (n.id === notifId) {
          return {
            ...n,
            message: `${n.message}\n\n-> Doanh nghiệp phản hồi thương thảo: "${feedbackMsg}"`,
            isRead: true,
            meta: {
              ...n.meta,
              actioned: true,
      ...
      setNotificationsList(updatedNotifs);
      localStorage.setItem('vietagri_notifications_customer', JSON.stringify(updatedNotifs));
      setNegotiationInputId(null);
      setNegotiationText('');
      setSuccessBanner(`Gửi ý kiến thương thảo cho hợp đồng ${contractNumber} thành công!`);
    } catch (e) {
      console.error(e);
    }
  };

  const openContractFromNotification = (contractNumber: string) => {
    const foundContract = contractsList.find(c => c.contractNo === contractNumber || c.id === contractNumber);
    if (foundContract) {
      setSelectedContractForModal(foundContract);
      setActiveTab('contracts');
      setShowNotificationsDropdown(false);
    } else {
      alert(`Hợp đồng ${contractNumber} không tìm thấy hoặc đang được xử lý.`);
    }
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setContractsList(apiContracts);
        
        // Sync to legacy localStorage for other pages if needed
        localStorage.setItem('vietagri_contracts_v2', JSON.stringify(apiContracts));
      } catch (error) {
  ...
  const handlePrintContract = (c: any) => {
    const includeStamp = window.confirm(
      ...
      "• Nhấn CANCEL để tải: Bản in SẠCH (bản ký tay truyền thống / không mộc / không in sẵn tên để ký tay và đóng dấu trực tiếp)."
    );
  ...
  const completeSigningCustomer = async (contractId: string) => {
    // Ket noi API
    try {
      const targetContract = contractsList.find(c => String(c.id) === String(contractId));
      if (!targetContract) return;

      const updatedData = { ...targetContract, status: 'active' };
      ...
      setSelectedContractForModal(updatedData);

      // 4. Update step to show completion details
      setSigningStep('success');
    } catch (error) {
      console.error("Failed to confirm signature via API:", error);
      alert("Lỗi khi ký số hợp đồng! Vui lòng thử lại.");
    }
  };
    ...
    const fetchMarketplace = async () => {
      ...
      try {
        ...
        const transformedPosts = posts.map((post: any) => ({
          ...
          tag: 'Mới đăng',
          price: post.price ? (post.price.includes('₫') ? post.price : post.price + ' ₫') : '0 ₫',
          unit: post.unit || 'kg',
        ...
        setProducts(transformedPosts);
      } catch (error) {
        console.error("Failed to fetch marketplace posts via API:", error);
      }
    };
    fetchMarketplace();
  }, []);
  ...
  const handleMockPayment = async () => {
    ...
    // Ket noi API
    try {
        const target = updated.find(u => String(u.id) === String(contractId));
        if (target) {
            await contractService.updateContract(contractId, target);
        }
    } catch (error) {
        console.error("Failed to update payment via API:", error);
    }
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex font-sans">
      ...
      <main className="flex-1 overflow-y-auto pb-24 xl:pb-0">
        <header className="bg-white/90 backdrop-blur-xl sticky top-0 z-40 px-4 py-4 md:px-10 md:py-8 flex items-center justify-between border-b border-slate-100">
          <div className="flex items-center gap-6">
             <Link to="/" className="xl:hidden flex items-center gap-2">
               <div className="w-10 h-10 bg-[#004d40] rounded-xl flex items-center justify-center shadow-md">
                 <span className="text-white font-black text-xl">V</span>
               </div>
               <span className="text-[#004d40] font-black tracking-wider text-sm">VIETAGRI</span>
             </Link>
          ...
          <div className="flex items-center gap-4 md:gap-8">
            <div className="relative">
              <button 
                ...
                className={`relative text-slate-500 hover:text-[#004d40] transition-colors p-2 rounded-xl hover:bg-slate-50 cursor-pointer ${showNotificationsDropdown ? 'text-[#004d40] bg-slate-50' : ''}`}
              ...
              <AnimatePresence>
                {showNotificationsDropdown && (
                  <>
                    {/* Backdrop cover for mobile closing */}
                    <div 
                    ...
                    <motion.div
                      ...
                      className="fixed left-4 right-4 top-20 md:absolute md:left-auto md:right-0 md:top-auto md:mt-3 md:w-[420px] bg-white rounded-2xl shadow-2xl border border-slate-100 z-50 overflow-hidden font-sans origin-top-right text-left"
                    >
                      {/* Dropdown Header */}
                      <div className="p-3.5 bg-[#004d40] text-white flex items-center justify-between">
                        <div className="flex items-center gap-1.5">
                          <Bell size={15} className="text-[#4ade80]" />
                          <h4 className="font-extrabold text-[11px] tracking-wider uppercase text-emerald-50">Thông báo</h4>
                        </div>
                        <div className="flex items-center gap-2">
                          {notificationsList.filter(n => !n.isRead).length > 0 && (
                            <button 
                              onClick={markAllAsRead}
                              className="text-[9px] bg-white/15 hover:bg-white/25 px-2 py-0.5 rounded-md font-bold transition-all uppercase tracking-wide cursor-pointer text-emerald-100"
                            >
                              Đọc tất cả
                            </button>
                          )}
                      ...
                      {/* Notification list wrapper container */}
                      <div className="max-h-[350px] overflow-y-auto divide-y divide-slate-100">
                        {notificationsList.length === 0 ? (
                          <div className="py-10 px-6 text-center text-slate-400">
                            <CheckCircle2 size={32} className="mx-auto mb-2.5 text-slate-200" />
                            <p className="text-xs font-bold">Hộp nhật ký thông báo trống!</p>
                            <p className="text-[10px] text-slate-300 mt-0.5">Không có tin tức hoặc thông tin mới nào.</p>
                          </div>
                        ) : (
                          notificationsList.map((notif) => (
                            <div 
                              ...
                              className={`p-3.5 transition-colors relative hover:bg-slate-50/80 cursor-pointer ${!notif.isRead ? 'bg-emerald-50/15' : ''}`}
                            >
                              {/* Indicator block for unread */}
                              {!notif.isRead && (
                              ...
                              <div className="flex items-start justify-between gap-3 pl-1.5">
                                <div className="space-y-1 flex-1 min-w-0">
                                  {/* Notification Tag representing Type */}
                                  <div className="flex items-center gap-2">
                                    <span className={`text-[8px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded-md ${
                                      notif.type === 'negotiation' ? 'bg-purple-50 text-purple-700 border border-purple-100' :
                                      notif.type === 'rejection' ? 'bg-rose-50 text-rose-700 border border-rose-100' :
                                      notif.type === 'approval' ? 'bg-amber-50 text-amber-700 border border-amber-100' :
                                      'bg-emerald-50 text-emerald-700 border border-emerald-100'
                                    }`}>
                                      {notif.type === 'negotiation' ? 'Thương thảo' :
                                       notif.type === 'rejection' ? 'Từ chối' :
                                       notif.type === 'approval' ? 'Phê duyệt' : 'Phản hồi'}
                                    </span>
                                    {notif.contractNo && (
                                      <span className="text-[8px] font-mono font-bold text-slate-400">
                                        Mã số HĐ: {notif.contractNo}
                                      </span>
                                    )}
                                  </div>

                                  <h5 className="font-extrabold text-[11px] md:text-xs text-[#004d40] tracking-tight">{notif.title}</h5>
                                  <p className="text-[10px] md:text-[11px] text-slate-500 leading-relaxed font-medium break-words whitespace-pre-line">{notif.message}</p>

                                  <div className="text-[8px] text-slate-400 font-bold flex items-center gap-1.5 mt-1.5">
                                    <Clock size={8} /> {new Date(notif.timestamp).toLocaleTimeString('vi-VN')} - {new Date(notif.timestamp).toLocaleDateString('vi-VN')}
                                  </div>
                                </div>

                                <button 
                                  onClick={(notifE) => deleteNotification(notif.id, notifE)}
                                  className="text-slate-300 hover:text-red-500 p-1 rounded-md hover:bg-slate-100 transition-colors shrink-0 cursor-pointer"
                                  title="Xóa thông báo"
                                >
                                  <X size={12} />
                                </button>
                      ...
                      <div className="p-2.5 bg-slate-50 border-t border-slate-100 text-center">
                        <span className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">Hệ thống Giám sát VietAgri v2.0</span>
                      </div>
                    </motion.div>
                  </>
                )}
            ...
            <Link 
              to="/customer/profile"
              className="flex items-center gap-3 pl-3 md:pl-8 border-l border-slate-100 hover:opacity-80 transition-opacity"
            >
              ...
              <div className="w-10 h-10 md:w-12 md:h-12 bg-slate-100 rounded-xl md:rounded-2xl overflow-hidden shadow-inner border-2 border-white flex items-center justify-center text-[#004d40] font-black italic border-slate-200">
                {userData?.avatar ? (
          ...
          <AnimatePresence mode="wait">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-8">
                  {products.map((product) => (
                    <motion.div 
                      ...
                      className="bg-white rounded-2xl md:rounded-[2.5rem] overflow-hidden shadow-md md:shadow-xl shadow-slate-200/50 border border-slate-100 relative group flex flex-col h-full cursor-pointer"
                    >
                      <div className="h-36 sm:h-48 md:h-64 relative overflow-hidden shrink-0">
                        <img src={product.image} alt={product.name} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                        <div className="absolute top-2 left-2 md:top-4 md:left-4 bg-white/90 backdrop-blur-md px-2 py-0.5 md:px-3 md:py-1 rounded-full text-[8px] md:text-[9px] font-black uppercase tracking-widest text-[#004d40]">
                          {product.tag}
                        </div>
                        <button className="absolute top-2 right-2 md:top-4 md:right-4 w-7 h-7 md:w-10 md:h-10 bg-white/90 backdrop-blur-md rounded-full flex items-center justify-center text-slate-400 hover:text-red-500 transition-colors">
                      ...
                      <div className="p-3 md:p-8 flex-1 flex flex-col">
                        <div className="flex items-center justify-between gap-1 mb-1 md:mb-2">
                          <p className="text-[8px] md:text-[10px] text-slate-400 font-bold uppercase tracking-widest flex items-center gap-1 md:gap-2 truncate">
                            <MapPin size={10} className="text-[#4ade80] shrink-0" /> <span className="truncate">{product.origin}</span>
                          </p>
                          {product.htxId && (
                            <span className="text-[8px] md:text-[10px] font-black text-[#004d40] bg-[#004d40]/5 px-1.5 py-0.5 rounded italic shrink-0">
                              {product.htxId}
                            </span>
                          )}
                        </div>
                        <h4 className="text-xs md:text-lg font-black text-[#004d40] mb-1.5 md:mb-2 leading-snug md:leading-tight min-h-[2.2rem] md:min-h-[3rem] line-clamp-2">{product.name}</h4>
                        
                        {product.htxName && (
                          <div className="flex items-center gap-1 md:gap-2 mb-2 md:mb-4">
                            <Building2 size={10} className="text-slate-400 shrink-0" />
                            <p className="text-[8px] md:text-[10px] font-bold text-slate-500 uppercase tracking-wider truncate">{product.htxName}</p>
                          </div>
                        )}

                        <div className="mt-auto flex items-center justify-between gap-1">
                          <div className="flex-1 text-left min-w-0">
                            <p className="text-[8px] md:text-[10px] font-black text-[#4ade80] uppercase tracking-widest mb-0.5">Giá niêm yết</p>
                            <p className="text-xs md:text-lg font-black text-[#004d40] truncate">
                              {product.price}
                              <span className="text-[9px] md:text-xs text-slate-400 font-bold ml-0.5">/{product.unit}</span>
                            </p>
                          </div>
                          <button 
                            onClick={() => setSelectedProduct(product)}
                            className="w-8 h-8 md:w-12 md:h-12 bg-slate-50 group-hover:bg-[#004d40] transition-colors rounded-xl md:rounded-2xl flex items-center justify-center text-[#004d40] group-hover:text-white shadow-inner cursor-pointer shrink-0"
                          >
            ...
            {activeTab === 'traceability' && (
              <motion.div 
                ...
                exit={{ opacity: 0, x: -20 }}
                className="max-w-4xl mx-auto py-6 md:py-12"
              >
                <div className="bg-white rounded-3xl md:rounded-[4rem] p-6 md:p-16 shadow-2xl border border-slate-50 text-center relative overflow-hidden">
                   <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-[#4ade80] via-[#004d40] to-[#4ade80]" />
                   <div className="w-20 h-20 md:w-24 md:h-24 bg-[#004d40]/5 rounded-2xl md:rounded-[2rem] flex items-center justify-center mx-auto mb-6 md:mb-8 text-[#004d40]">
                      <QrCode size={40} className="md:h-12 md:w-12" />
                   </div>
                   <h2 className="text-2xl md:text-4xl font-black text-[#004d40] mb-4 md:mb-6 tracking-tight">Cổng Truy Xuất Minh Bạch</h2>
                   <p className="text-slate-500 font-medium text-sm md:text-lg mb-8 md:mb-12 max-w-xl mx-auto leading-relaxed">Mỗi sản phẩm từ VIETAGRI đều mang một định danh số duy nhất. Quét mã QR trên bao bì để xem lịch sử hành trình từ hạt giống đến bàn ăn.</p>
                   
                   <div className="bg-slate-50 rounded-2xl md:rounded-[3rem] p-6 md:p-12 border-2 border-dashed border-slate-200 mb-8 md:mb-12">
                      <div className="w-36 h-36 md:w-48 md:h-48 bg-white p-4 rounded-2xl md:rounded-3xl mx-auto shadow-2xl border border-slate-100 flex items-center justify-center mb-6 md:mb-8">
                         <QrCode size={100} className="text-[#004d40] opacity-80" />
                      </div>
                      <p className="text-[10px] md:text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-6 md:mb-8">Nhập mã định danh sản phẩm</p>
                      <div className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
                        <input type="text" placeholder="Ví dụ: VA-2024-8891" className="flex-1 px-5 py-4 bg-white rounded-xl md:rounded-2xl border border-slate-100 outline-none font-bold text-sm md:font-black text-[#004d40] shadow-sm focus:border-[#004d40] transition-all" />
                        <button className="bg-[#004d40] text-white px-6 py-4 rounded-xl md:rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-[#4ade80] transition-all shadow-xl shadow-[#004d40]/20 cursor-pointer">Kiểm tra</button>
                      </div>
                   </div>
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
                      {[
                        ...
                        <div key={i} className="space-y-3 bg-slate-50/50 p-4 rounded-2xl md:bg-transparent md:p-0 md:rounded-none">
                           <div className="text-[#4ade80] inline-block p-4 bg-[#4ade80]/5 rounded-2xl mb-1 md:mb-2">{item.icon}</div>
              ...
              {signingStep === 'idle' && (
                <>
                  ...
                  {selectedContractForModal.status === 'Thương thảo' && (
                    <div className="p-6 bg-yellow-50 border-t border-yellow-100 flex flex-col sm:flex-row items-center justify-between gap-4 shrink-0 px-8 rounded-b-[3rem] w-full">
                      <div className="flex items-start gap-2 max-w-full min-w-0 flex-1">
                        <AlertCircle className="text-yellow-600 mt-1 shrink-0" size={18} />
                        <div className="text-xs text-slate-500 font-bold leading-normal min-w-0 flex-1">
                          <p>Ban quản trị Hợp tác xã đang đề xuất Thương thảo để điều chỉnh các điều khoản hợp đồng. Vui lòng liên hệ trực tiếp để thống nhất nội dung sửa đổi.</p>
                          {selectedContractForModal.negotiationNotes && (
                            <div className="mt-2 p-3.5 bg-white rounded-2xl border border-yellow-200 text-yellow-800 font-mono italic break-words max-w-full">
                              <strong>Ý kiến từ Ban quản trị:</strong> "{selectedContractForModal.negotiationNotes}"
                            </div>
                          )}
                        </div>
                      </div>
                  ...
                  {(selectedContractForModal.status === 'Hợp đồng đã bị từ chối' || selectedContractForModal.status === 'rejected_super_admin' || selectedContractForModal.status === 'rejected') && (
                    <div className="p-6 bg-rose-50 border-t border-rose-100 flex flex-col sm:flex-row items-center justify-between gap-4 shrink-0 px-8 rounded-b-[3rem] w-full">
                      <div className="flex items-start gap-2 max-w-full min-w-0 flex-1">
                        <AlertCircle className="text-rose-500 mt-1 shrink-0" size={18} />
                        <div className="text-xs text-slate-500 font-bold leading-normal min-w-0 flex-1">
                          <p>Hợp đồng này đã bị Ban quản trị Từ chối phê duyệt. Quý khách vui lòng liên hệ trực tiếp hoặc tạo lại hồ sơ hợp đồng mới đúng quy chuẩn.</p>
                          {selectedContractForModal.rejectReason && (
                            <div className="mt-2 p-3.5 bg-white rounded-2xl border border-rose-200 text-rose-800 font-mono italic break-words max-w-full">
                              <strong>Lý do từ chối:</strong> "{selectedContractForModal.rejectReason}"
                            </div>
                          )}
                        </div>
                      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-md border-t border-slate-100 shadow-[0_-8px_30px_rgba(0,0,0,0.06)] xl:hidden px-4 py-2.5 flex items-center justify-around pb-[calc(0.75rem+env(safe-area-inset-bottom))]">
        {[
          { id: 'marketplace', label: 'Cửa hàng', icon: <ShoppingBag size={20} /> },
          { id: 'contracts', label: 'Hợp đồng', icon: <FileText size={20} /> },
          { id: 'traceability', label: 'Truy xuất', icon: <QrCode size={20} /> },
          { id: 'orders', label: 'Đơn hàng', icon: <Truck size={20} /> },
          { id: 'feedback', label: 'Đánh giá', icon: <Star size={20} /> },
        ].map((item) => (
          <button
            key={item.id}
            onClick={() => {
            ...
            className={`flex flex-col items-center gap-1 py-1 px-3 rounded-2xl transition-all cursor-pointer ${
              activeTab === item.id 
                ? 'text-[#004d40] font-black scale-105' 
                : 'text-slate-400 font-medium hover:text-slate-600'
            }`}
          >
            <div className={`p-1.5 rounded-xl transition-all ${
              activeTab === item.id ? 'bg-emerald-50 text-[#004d40] shadow-sm shadow-emerald-950/5' : ''
            }`}>
              {item.icon}
            </div>
