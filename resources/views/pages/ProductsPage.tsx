    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const transformedPosts = posts.map((post: any) => ({
          ...
          price: post.price ? (post.price.toString().includes('đ') ? post.price : post.price + 'đ') : '0đ'
        }));

        setAllProducts([...transformedPosts, ...staticProducts]);
      } catch (error) {
            ...
            <AnimatePresence mode='popLayout'>
              {filteredProducts.map((product) => (
                <motion.div
                  ...
                  viewport={{ once: true }}
                  className="group bg-white rounded-2xl md:rounded-[2.5rem] overflow-hidden shadow-sm hover:shadow-[0_20px_50px_rgba(0,0,0,0.06)] transition-all duration-500 border border-slate-100 relative flex flex-col h-full"
                >
                  {/* Badges */}
                  <div className="absolute top-2 left-2 md:top-6 md:left-6 z-10 flex flex-col gap-1">
                    <div className="bg-white/90 backdrop-blur-sm text-amber-500 text-[8px] md:text-[9px] font-black px-2 py-0.5 md:px-3 md:py-1.5 rounded-full shadow-sm flex items-center gap-1.5 border border-amber-500/20 uppercase tracking-wider">
                      <Star size={10} fill="currentColor" className="shrink-0" />
                      <span className="hidden xs:inline">Chuẩn</span> {product.ocop}
                    </div>
                  </div>
                  <div className="absolute top-2 right-2 md:top-6 md:right-6 z-10">
                    <div className="bg-[#059669]/90 backdrop-blur-sm px-2 py-0.5 md:px-3 md:py-1.5 rounded-full text-[8px] md:text-[9px] font-black text-white flex items-center gap-1.5 shadow-sm uppercase tracking-wider">
                      <Leaf size={10} className="shrink-0" />
                      <span className="hidden xs:inline">GIS Origin</span>
                      <span className="xs:hidden">GIS</span>
                    </div>
                  </div>

                  {/* Image Container */}
                  <div className="h-36 sm:h-56 md:h-72 overflow-hidden relative shrink-0">
                    <img 
                      src={product.image} 
                      alt={product.name}
                    ...
                    {/* Hover Overlay Buttons (Hidden on mobile for better touch experience) */}
                    <div className="absolute inset-0 bg-[#004d40]/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300 hidden md:flex items-center justify-center">
                      <Link 
                  ...
                  <div className="p-3 md:p-8 flex-1 flex flex-col">
                    <h3 className="text-xs md:text-xl font-bold text-[#004d40] mb-2 md:mb-3 group-hover:text-[#4ade80] transition-colors line-clamp-2 h-8 md:h-14">
                      {product.name}
                    </h3>

                    {/* Short Description */}
                    {product.desc && (
                      <p className="text-[10px] md:text-xs text-slate-400 line-clamp-2 mb-2 leading-relaxed font-medium hidden sm:block">
                        {product.desc}
                      </p>
                    )}
                    ...
                    <div className="mt-auto">
                      <Link 
                        to={`/product/${product.id}`}
                        className="block w-full py-2 md:py-4 border border-2 border-[#004d40]/10 hover:border-[#4ade80] hover:bg-[#4ade80] hover:text-white text-[#004d40] text-[8px] md:text-[11px] font-black uppercase tracking-widest rounded-xl md:rounded-2xl transition-all text-center"
                      >
                        Chi tiết <span className="hidden sm:inline">sản phẩm</span>
                      </Link>
                    </div>
                  </div>
                </motion.div>
