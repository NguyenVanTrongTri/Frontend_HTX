...
export default function ContractPage() {
  ...
  const [contract, setContract] = useState<{
    ...
    date: '',
    location: '',
    product: { id: '', name: '', unit: '', quantity: '5000', price: '', note: '' },
    totalInWords: '',
    paymentDate: '',
    paymentPhase1: '30%',
    paymentPhase2: '70%',
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Since getPosts might return an object or array, handle accordingly
        const salePosts = Array.isArray(response) ? response : (response?.data || []);
        foundPost = salePosts.find((p: any) => String(p.id) === String(productId));
      } catch (e) {
        console.error("Failed to fetch sale posts:", e);
      }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Still updating localStorage for immediate UI feedback in other parts of the app if they haven't been updated to API yet
      localStorage.setItem('vietagri_contracts_v2', JSON.stringify([newContractV2, ...(JSON.parse(localStorage.getItem('vietagri_contracts_v2') || '[]'))]));
      localStorage.setItem('vietagri_contracts_v3', JSON.stringify([newContractV2, ...(JSON.parse(localStorage.getItem('vietagri_contracts_v3') || '[]'))]));
      ...
      localStorage.setItem('vietagri_contracts', JSON.stringify([newContractSimple, ...(JSON.parse(localStorage.getItem('vietagri_contracts') || '[]'))]));

      // Show beautiful success overlay
      setSavedContract(newContractV2);
      setShowSuccessModal(true);
    } catch (e) {
      console.error("Failed to create contract via API:", e);
          ...
          <div className="text-center mb-10">
            <h1 className="text-xl font-black uppercase text-[#004d40]">
              {formattedContractNumber?.includes('HĐHTSXNN') 
                ...
                : (formattedContractNumber?.includes('HDLKTMHH') 
