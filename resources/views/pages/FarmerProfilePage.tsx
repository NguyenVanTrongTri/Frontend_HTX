...
export default function FarmerProfilePage() {
    ...
    useEffect(() => {
        const loadProfile = async () => {
            ...
            try {
                ...
                // If user is logged in, we use their auth profile as base
                const fullProfile = {
                    ...user,
                    memberId: user.memberId || user.id || `XV-${user.phone}`,
                    name: user.name || 'Thành viên HTX',
                    dob: user.dob || '',
                    ...
                    capitalContribution: user.capitalContribution || '0',
                    capitalRatio: user.capitalRatio || '0',
                    contributionDate: user.contributionDate || '',
                    phone: user.phone,
                    email: user.email || '',
                    area: user.area || '0',
                    location: user.location || '',
                    crop: user.crop || 'Cà phê Arabica',
                    qualityStandard: user.qualityStandard || 'VietGAP',
                    bankAccNumber: user.bankAccNumber || '',
                    bankName: user.bankName || '',
                    bankAccHolder: user.bankAccHolder || (user.name || '').toUpperCase(),
                    cooperativeId: user.cooperativeId || 'HTX-001',
                    htxName: user.htxName || ''
                };

                const htx = COOPERATIVES.find(c => c.id === fullProfile.cooperativeId);
                fullProfile.htxName = htx ? htx.name : (fullProfile.htxName || 'HTX Cà phê Cầu Đất');

                setFarmer(fullProfile);
                setFormData(fullProfile);
            } catch (err) {
                console.error("Error loading profile via API:", err);
                // Fallback to minimal data if API fails to prevent white screen
                const phone = localStorage.getItem('userPhone');
                if (phone) {
                   const fallback = { name: 'Nông dân', phone, dob: '', gender: 'Nam', nationality: 'VN', permanentAddress: '', contactAddress: '', cccd: '', idDate: '', idPlace: '', joiningDate: '', terminationDate: '---', capitalContribution: '0', capitalRatio: '0', contributionDate: '', email: '', area: '0', location: '', crop: 'Cà phê', qualityStandard: 'VietGAP', bankAccNumber: '', bankName: '', bankAccHolder: '', cooperativeId: 'HTX-001', htxName: 'HTX Cà phê Cầu Đất', memberId: `XV-${phone}` };
                   setFarmer(fallback);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // Sync local storage for other legacy parts of the app
