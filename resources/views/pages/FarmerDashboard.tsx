...
export default function FarmerDashboard() {
  ...
  const handleConfirmDelivery = async () => {
    ...
    // Ket noi API
    try {
      await harvestService.createDelivery(deliveryRecord);
      
      // Update harvest schedule status
      await harvestService.createSchedule({...deliveryModal, status: 'delivered', isDelivered: true});
    } catch (error) {
      console.error("Failed to send delivery via API:", error);
    }

    // Save to pending QC deliveries for Admin (Fallback)
  ...
  const confirmHarvest = async (id: string) => {
    const updatedSchedules = scheduledHarvests.map((s: any) => s.id === id ? {...s, confirmed: true} : s);
    setScheduledHarvests(updatedSchedules);

    // Ket noi API
    try {
        const schedule = scheduledHarvests.find(s => s.id === id);
        if (schedule) {
            await harvestService.createSchedule({...schedule, confirmed: true});
        }
    } catch (error) {
        console.error("Failed to confirm harvest via API:", error);
    }
  ...
  useEffect(() => {
    const fetchData = async () => {
      // Ket noi API
      try {
        const [contractsResponse, schedulesResponse] = await Promise.all([
          contractService.getAllContracts(),
          harvestService.getAllSchedules()
        ]);

        const apiContracts = Array.isArray(contractsResponse) ? contractsResponse : (contractsResponse?.data || []);
        const apiSchedules = Array.isArray(schedulesResponse) ? schedulesResponse : (schedulesResponse?.data || []);

        setAllContractsList(apiContracts);
        
        const phone = localStorage.getItem('userPhone') || '';
        const name = farmerName; // Use current state or fetch from auth

        // Filter schedules
        setScheduledHarvests(apiSchedules.filter((s: any) => s.farmer === name || s.phone === phone));

        // Filter contracts
        const farmerContracts = apiContracts.filter((c: any) => {
        ...
        console.error("Failed to fetch dashboard data via API:", error);
      }
    };
    fetchData();
  ...
  useEffect(() => {
    const phone = localStorage.getItem('userPhone') || '';
    const name = farmerName || 'Thành viên HTX';
    
    // Read list of deleted notifications
    const deletedRaw = localStorage.getItem('vietagri_notif_deleted_ids');
    ...
    const readMapRaw = localStorage.getItem('vietagri_notif_read_map');
    let readMap: Record<string, boolean> = {};
    if (readMapRaw) {
      try {
        readMap = JSON.parse(readMapRaw);
      } catch (e) {}
    }

    const dynamicList: any[] = [];

    // 1. New Crop Approval Requests & Reports from pending_registration_members
    const rawPending = localStorage.getItem('pending_registration_members');
    if (rawPending) {
      try {
        const pendingItems = JSON.parse(rawPending);
        const myItems = pendingItems.filter((m: any) => 
          String(m.phone || '').replace(/^0+/, '') === String(phone).replace(/^0+/, '')
        );
        myItems.forEach((m: any) => {
          const cropName = m.cropName || m.name || 'Cây trồng';
          const nid = `crop-${m.type || 'approval'}-${m.id || cropName}-${m.status || 'pending'}`;
          const isRead = !!readMap[nid];

          if (m.type === 'crop_approval') {
            if (m.status === 'approved') {
              dynamicList.push({
                id: nid,
                type: 'success',
                text: `Phê duyệt thêm cây trồng: ${cropName}`,
                time: 'Mới đây',
                read: isRead,
                desc: `Yêu cầu trồng mới cây ${cropName} (Diện tích: ${m.area || ''}) đã được HTX phê duyệt thành công! Bạn có thể bắt đầu cập nhật nhật ký sinh trưởng.`
              });
            } else if (m.status === 'rejected') {
              dynamicList.push({
                id: nid,
                type: 'warning',
                text: `Yêu cầu thêm cây trồng bị từ chối`,
                time: 'Vừa xong',
                read: isRead,
                desc: `Yêu cầu trồng mới cây ${cropName} đã bị HTX từ chối. Vui lòng liên hệ ban liên kết nông sản HTX để biết lý do.`
              });
            } else {
              dynamicList.push({
                id: nid,
                type: 'info',
                text: `Đăng ký trồng mới cây ${cropName}`,
                time: 'Chờ duyệt',
                read: isRead,
                desc: `Yêu cầu của bạn về gieo trồng mới cây ${cropName} (Diện tích: ${m.area || ''}) đã được ghi nhận và đang chờ duyệt.`
              });
            }
          } else if (m.type === 'crop_report') {
            if (m.status === 'approved') {
              dynamicList.push({
                id: nid,
                type: 'success',
                text: `Đồng ý báo cáo gieo trồng cây ${cropName}`,
                time: 'Mới đây',
                read: isRead,
                desc: `Báo cáo sinh trưởng giai đoạn ${m.growthStage || 'Phát triển'} (Tình trạng sức khỏe: ${m.health || 'Tốt'}) cho cây ${cropName} đã được phê duyệt.`
              });
            } else if (m.status === 'rejected') {
              dynamicList.push({
                id: nid,
                type: 'warning',
                text: `Từ chối báo cáo: ${cropName}`,
                time: 'Vừa xong',
                read: isRead,
                desc: `Báo cáo sinh trưởng cho cây ${cropName} chưa đạt yêu cầu của trạm kiểm duyệt. Đề nghị cải thiện và báo cáo lại.`
              });
            } else {
              dynamicList.push({
                id: nid,
                type: 'info',
                text: `Báo cáo sinh trưởng cây ${cropName}`,
                time: 'Chờ duyệt',
                read: isRead,
                desc: `Báo cáo sinh trưởng cây ${cropName} giai đoạn ${m.growthStage || ''} đang đợi ban nông nghiệp HTX thẩm định.`
              });
            }
          }
        });
      } catch (e) {
        console.error("Error parsing pending items for notifications:", e);
      }
    }

    // 2. Agricultural support requests from vietagri_farmer_requests
    const rawRequests = localStorage.getItem('vietagri_farmer_requests');
    if (rawRequests) {
      try {
        const parsedRequests = JSON.parse(rawRequests);
        const myReqs = parsedRequests.filter((r: any) => 
          String(r.farmerPhone || r.phone || '').replace(/^0+/, '') === String(phone).replace(/^0+/, '') ||
          r.farmer === name
        );
        myReqs.forEach((r: any) => {
          const reqString = r.custom || (r.requests && r.requests.join(', ')) || 'Hỗ trợ nông nghiệp';
          const nid = `req-${r.id}-${r.status || 'pending'}`;
          const isRead = !!readMap[nid];
          ...
          if (r.status === 'approved') {
            dynamicList.push({
              id: nid,
              type: 'success',
              text: `Phê duyệt yêu cầu hỗ trợ nông nghiệp`,
              time: 'Vừa xong',
              read: isRead,
              desc: `Yêu cầu hỗ trợ "${reqString}" của quý hộ đã được phê duyệt điều phối cấp phát thành công từ đại diện HTX.`
            });
          } else if (r.status === 'rejected') {
            dynamicList.push({
              id: nid,
              type: 'warning',
              text: `Từ chối duyệt hỗ trợ vật tư`,
              time: 'Vừa xong',
              read: isRead,
              desc: `HTX từ chối phê duyệt đơn yêu cầu hỗ trợ "${reqString}" của bạn do tình hình nhân lực/vật tư kho bãi hiện hữu.`
            });
          } else {
            dynamicList.push({
              id: nid,
              type: 'info',
              text: `Yêu cầu hỗ trợ đã gửi lên hệ thống`,
              time: 'Đang chờ',
              read: isRead,
              desc: `Yêu cầu hỗ trợ dịch vụ/nông nghiệp "${reqString}" của bạn đang tiến hành xếp hàng chờ bộ phận kho bãi kiểm duyệt.`
            });
          }
        });
      } catch (e) {
        console.error("Error parsing farmer requests for notifications:", e);
      }
    }

    // 3. Harvest Schedules from vietagri_harvest_schedules
    const rawSchedules = localStorage.getItem('vietagri_harvest_schedules');
    if (rawSchedules) {
      try {
        const parsedSchedules = JSON.parse(rawSchedules);
        const mySchedules = parsedSchedules.filter((s: any) => 
          s.farmer === name || String(s.phone || '').replace(/^0+/, '') === String(phone).replace(/^0+/, '')
        );
        mySchedules.forEach((s: any) => {
          const nid = `harvest-${s.id}-${s.confirmed ? 'confirmed' : 'pending'}`;
          const isRead = !!readMap[nid];
          ...
          if (s.confirmed) {
            dynamicList.push({
              id: nid,
              type: 'success',
              text: `Lịch thu mua / Thu hoạch đã xác nhận`,
              time: 'Đã ký',
              read: isRead,
              desc: `Đã xác nhận thành công lịch thu mua cho sản phẩm ${s.product} - Sản lượng: ${s.qty} ${s.unit || 'tấn'} vào ngày ký kết.`
            });
          } else {
            dynamicList.push({
              id: nid,
              type: 'info',
              text: `Lệnh thu mua & Thu hoạch nông sản mới`,
              time: 'Mới nhận',
              read: isRead,
              desc: `HTX tạo lịch thu mua dự kiến sản phẩm ${s.product} (Sản lượng: ${s.qty} ${s.unit || 'tấn'}) dự kiến lúc ${s.expectedHarvestDate || s.harvestDate}. Vui lòng bấm vào để xác nhận lịch.`
            });
          }
        });
      } catch (e) {
        console.error("Error parsing schedules for notifications:", e);
      }
    }

    // 4. Contracts from localStorage (combined v1, v2, v3)
    try {
      const getContractsCombined = () => {
        let combined: any[] = [];
        const rawV1 = localStorage.getItem('vietagri_contracts');
        const rawV2 = localStorage.getItem('vietagri_contracts_v2');
        if (rawV1) {
          try { combined = [...combined, ...JSON.parse(rawV1)]; } catch(e){}
        }
        if (rawV2) {
          try { combined = [...combined, ...JSON.parse(rawV2)]; } catch(e){}
        }
        return combined;
      };

      const allC = getContractsCombined();
      const myC = allC.filter((c: any) => {
        const sellerPhone = String(c.seller?.phone || c.phone || '').replace(/^0+/, '');
        const myPhone = phone.replace(/^0+/, '');
        return sellerPhone === myPhone && myPhone !== '';
      });
      ...
      myC.forEach((c: any) => {
        const ccode = c.id || c.code || 'HD-NEW';
        const prod = c.commercialProductName || c.productName || 'Nông sản';
        const nid = `contract-${ccode}-${c.status || 'pending'}`;
        const isRead = !!readMap[nid];

        if (c.status === 'signed' || c.status === 'approved' || c.partyASigned || c.signed) {
          dynamicList.push({
            id: nid,
            type: 'success',
            text: `Hợp đồng liên kết bao tiêu đã ký kết`,
            time: 'Thành công',
            read: isRead,
            desc: `Hợp đồng bao tiêu nông sản số hiệu ${ccode} cho mặt hàng ${prod} đã chính thức được phê duyệt có hiệu lực vụ mùa này.`
          });
        } else {
          dynamicList.push({
            id: nid,
            type: 'info',
            text: `Yêu cầu ký kết hợp đồng nông sản mới`,
            time: 'Chờ ký',
            read: isRead,
            desc: `Hợp đồng liên kết bao tiêu mới số hiệu ${ccode} ứng cho sản phẩm ${prod} đang chờ đại diện hộ gieo trồng ký kết.`
          });
        }
      });
    } catch (e) {
      console.error("Error parsing contracts for notifications:", e);
    }

    // Baseline alert configurations
    const staticAnnouncements = [
      { 
        id: 'warn-weather-extreme', 
        type: 'warning', 
        text: 'Cảnh báo thời tiết: Mưa lớn cực đoan kéo dài', 
        time: '2 ngày trước', 
        read: !!readMap['warn-weather-extreme'], 
        desc: 'Ban quản lý nông vụ cảnh báo đợt mưa lớn cực đoan có khả năng kèm gió giật mạnh. Bà con nông dân cần lưu ý chằng chống giàn và khơi thông hố thoát.' 
      },
      { 
        id: 'info-fertilizer-durian', 
        type: 'info', 
        text: 'Khuyến cáo bón phân cân đối cho sầu riêng', 
        time: '3 ngày trước', 
        read: !!readMap['info-fertilizer-durian'], 
        desc: 'Đang bước vào giai đoạn quyết định nuôi quả non sầu riêng. Hộ trồng nên áp dụng liều lượng phân lân cân đối tránh rụng sinh lý quả.' 
      },
      { 
        id: 'success-general-approval', 
        type: 'success', 
        text: 'Chính sách trợ giá vận chuyển nông vụ niên khóa mới', 
        time: '5 ngày trước', 
        read: !!readMap['success-general-approval'], 
        desc: 'Ban giám đốc HTX chính thức phê duyệt giảm phí ủy thác cơ giới hóa và hỗ trợ 10% cước xe tải chở nông sản cho các hợp hội thành viên.' 
      }
    ];

    const finalMerged = [...dynamicList, ...staticAnnouncements].filter(n => !deletedIds.includes(n.id));
    
    // Unread item sort bias
  ...
  }, [currentFarmerPhone, farmerName, dataRefreshKey]);

  const handleMarkAllRead = () => {
    const readMapRaw = localStorage.getItem('vietagri_notif_read_map');
    const readMap = readMapRaw ? JSON.parse(readMapRaw) : {};
    notifications.forEach(n => {
      readMap[n.id] = true;
    });
    localStorage.setItem('vietagri_notif_read_map', JSON.stringify(readMap));
    setDataRefreshKey(prev => prev + 1);
  };
  ...
  const handleDeleteSingleNotif = (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    const deletedRaw = localStorage.getItem('vietagri_notif_deleted_ids');
    let deletedIds: string[] = [];
    if (deletedRaw) {
    ...
    localStorage.setItem('vietagri_notif_deleted_ids', JSON.stringify(deletedIds));
  ...
  useEffect(() => {
    ...
    const fetchActiveCrops = async () => {
        ...
        try {
            ...
            console.error("Failed to fetch active crops via API:", error);
            ...
            if (savedFarmersRaw) {
              ...
              if (myProfile && myProfile.crops) {
                  ...
                  // To keep it simple, I'll just set the list from fallback
                  setCropsList(phone === '001' ? [...baseList] : baseList.filter(c => c.farmerPhone === phone || c.farmerPhone === '001'));
              }
            }
        }
  ...
  const handleAddDiaryEntry = async () => {
    ...
    // Ket noi API
    try {
        const crop = cropsList.find(c => c.id === selectedCropForDiary.id);
        if (crop) {
            const updatedDiary = [entryToAdd, ...(crop.diary || [])];
            await farmerService.updateCrop(currentFarmerPhone, crop.id, { ...crop, diary: updatedDiary });
        }
    } catch (error) {
        console.error("Failed to update diary via API:", error);
    }
  ...
  const handleSendRequest = async () => {
      ...
      // Ket noi API
      try {
        await farmerService.createFarmerRequest(request);
      } catch (error) {
        console.error("Failed to send request via API:", error);
      }
  ...
  const handleConfirmCrop = async () => {
    ...
    // Ket noi API
    try {
      await memberService.createMember(cropEntry);
    } catch (error) {
      console.error("Failed to submit crop request via API:", error);
    }
          ...
          <div className="flex items-center gap-6">
            <div className="relative">
              <button 
                ...
                className="relative text-slate-400 hover:text-emerald-700 hover:bg-slate-50 transition-all p-2 rounded-xl flex items-center justify-center cursor-pointer"
                title={`${unreadCount} thông báo mới`}
              >
                <Bell size={22} className={unreadCount > 0 ? "text-[#059669] animate-bounce" : "text-slate-500"} />
                {unreadCount > 0 && (
                  <span className="absolute -top-1 -right-1 w-5.5 h-5.5 bg-red-500 text-white text-[9px] font-black rounded-full flex items-center justify-center border-2 border-white shadow-sm">
                    {unreadCount}
                  </span>
                )}
              </button>
              ...
              {showNotifMenu && (
                <div 
                  className="fixed left-4 right-4 top-20 md:absolute md:left-auto md:right-0 md:top-auto md:mt-3 md:w-96 bg-white rounded-3xl border border-slate-100 shadow-[0_20px_50px_rgba(80,70,50,0.15)] overflow-hidden z-50 animate-in fade-in slide-in-from-top-3 duration-200"
                >
                  {/* Dropdown Header */}
                  <div className="bg-gradient-to-r from-emerald-800 to-[#004d40] px-5 py-4 flex items-center justify-between text-white">
                    <div className="flex items-center gap-2">
                      <Bell size={18} />
                      <span className="font-bold text-sm">Thông báo gần đây</span>
                    </div>
                    {unreadCount > 0 && (
                      <button 
                        onClick={handleMarkAllRead}
                        className="text-[10px] font-black uppercase tracking-wider bg-white/10 hover:bg-white/20 active:bg-white/30 px-2.5 py-1.5 rounded-lg transition-all cursor-pointer"
                      >
                        Đọc tất cả
                      </button>
                    )}
                    ...
                    {notifications.length === 0 ? (
                      ...
                      notifications.map((notif) => (
                        <div 
                          ...
                          className={`p-4 hover:bg-slate-50/80 transition-all cursor-pointer flex gap-3.5 relative ${
                            !notif.read ? 'bg-emerald-50/20' : ''
                          }`}
                        >
                          {!notif.read && (
                            <span className="absolute top-4 right-4 w-2 h-2 bg-[#059669] rounded-full animate-ping" />
                          )}
                          <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${
                            notif.type === 'warning' ? 'bg-red-50 text-red-500' : notif.type === 'info' ? 'bg-blue-50 text-blue-500' : 'bg-emerald-50 text-emerald-600'
                          }`}>
                            {notif.type === 'warning' ? <AlertCircle size={18} /> : notif.type === 'info' ? <Info size={18} /> : <CheckCircle2 size={18} />}
                          </div>
                          <div className="min-w-0 flex-1">
                            <div className="flex items-start justify-between gap-2">
                              <p className={`text-xs leading-snug text-slate-800 ${!notif.read ? 'font-bold' : 'font-medium'}`}>
                                {notif.text}
                              </p>
                            </div>
                            ...
                            <div className="flex items-center justify-between gap-2 mt-2 pt-1 border-t border-slate-50">
                              <p className="text-[9px] text-slate-400 font-extrabold uppercase tracking-wider">{notif.time}</p>
                              <button
                                onClick={(e) => handleDeleteSingleNotif(notif.id, e)}
                                className="text-slate-400 hover:text-red-500 hover:bg-red-50 transition-all p-1.5 rounded-lg flex items-center gap-1 text-[10px] font-bold cursor-pointer"
                  ...
                  <div className="bg-slate-50 px-4 py-3 flex items-center justify-between border-t border-slate-100">
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">
                      VietAgri Digital Platform
                    </p>
                    <button 
                      onClick={() => setShowNotifMenu(false)}
                      className="text-xs hover:text-emerald-700 transition-colors font-bold text-slate-500 px-3 py-1 cursor-pointer"
                    >
                      Đóng
                    </button>
                  </div>
