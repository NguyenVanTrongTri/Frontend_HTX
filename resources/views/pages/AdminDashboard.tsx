...
export default function AdminDashboard() {
  ...
  const [notificationsList, setNotificationsList] = useState<any[]>([
    { id: '1', title: 'Đơn hàng mới', message: 'HTX Cầu Đất vừa gửi yêu cầu duyệt hồ sơ.', isRead: false, type: 'negotiation', timestamp: new Date().toISOString() },
    { id: '2', title: 'Phê duyệt tài khoản', message: 'Tài khoản thành viên mới đã được xác thực.', isRead: false, type: 'approval', timestamp: new Date().toISOString() },
    { id: '3', title: 'Từ chối cập nhật', message: 'Sản phẩm "Gạo ST25" đã bị từ chối cập nhật.', isRead: true, type: 'rejection', timestamp: new Date().toISOString() },
  ]);
  ...
  useEffect(() => {
    ...
    const fetchFarmers = async () => {
      try {
        ...
        if (savedFarmersRaw) {
          try {
            parsed = JSON.parse(savedFarmersRaw);
          } catch (e) {
            console.error(e);
          }
        }
        
        const DEFAULT_10_FARMERS = [
          { 
            id: '1', 
            name: 'Lê Văn Tám', 
            phone: '123', 
        ...
        const merged = Array.isArray(parsed) && parsed.length > 0 ? [...parsed] : [];
        DEFAULT_10_FARMERS.forEach((df: any) => {
          const idx = merged.findIndex((mf: any) => mf.name === df.name || mf.phone === df.phone);
          if (idx === -1) {
            merged.push(df);
          } else {
            merged[idx] = {
        ...
        localStorage.setItem('vietagri_active_farmers', JSON.stringify(merged));
        localStorage.setItem('mock_farmers', JSON.stringify(merged));
        setActiveFarmers(merged);
      } catch (e) {
  ...
  const [activeTab, setActiveTab ] = useState<'members' | 'reports' | 'qc' | 'contracts' | 'finances' | 'network'>('members');
  ...
  useEffect(() => {
    ...
    const fetchStaff = async () => {
      try {
        const response = await memberService.getStaffAccounts();
  ...
  const handleCreateStaffAccount = async (e: React.FormEvent) => {
    ...
    try {
      // Ket Noi API
      const createdMember = await memberService.createMember(newFarmer);
      setActiveFarmers([createdMember, ...activeFarmers]);
      setStaffAccounts(prev => [createdMember, ...prev]);
      setCreatedStaffResult(createdMember);
      setNewStaffAccount({
        name: '',
        phone: '',
        cccd: '',
      ...
      console.error("Failed to create staff member via API:", e);
    }
  };
  ...
  const updateContractField = (field: string, value: any) => {
  ...
  const updateDeliveryStatus = async (id: string, status: string) => {
    try {
      await harvestService.updateDeliveryStatus(id, status, deliveryStatusNote);
      setPendingDeliveries(prev => prev.map(d => d.id === id ? { ...d, status } : d));
      setToastMessage('Cập nhật trạng thái giao hàng thành công!');
    } catch (e) {
      console.error(e);
    }
  };

  const [approvedMemberInfo, setApprovedMemberInfo] = useState<any>(null);
  ...
  useEffect(() => {
    ...
    const fetchHarvests = async () => {
      try {
        const response = await harvestService.getAllSchedules();
  ...
  useEffect(() => {
    ...
    const fetchDeliveries = async () => {
      try {
        const response = await harvestService.getPendingDeliveries();
  ...
  useEffect(() => {
    const fetchContracts = async () => {
      try {
        ...
        const contractList = Array.isArray(response) ? response : (response?.data || []);
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      try {
        ...
        const transList = Array.isArray(response) ? response : (response?.data || []);
  ...
  // Available unique crop names for harvest list filtering
  const availableHarvestCrops = Array.from(
    new Set(
      htxActiveFarmers.flatMap(f => (f.crops || []).map((c: any) => c.name))
    )
  ).filter(Boolean).sort();

  const filteredFarmersForHarvest = htxActiveFarmers.filter(farmer => {
    // 1. Name search match
    const matchesName = farmer.name.toLowerCase().includes(memberSearchQuery.toLowerCase());
    if (!matchesName) return false;

    // 2. Crop selection match
    if (harvestFilterCrop === 'all') return true;
    return (farmer.crops || []).some((c: any) => c.name === harvestFilterCrop);
  });
  ...
  // Ket Noi API: contract and harvest sync logic replaced by API calls in previous useEffects.
  ...
  const handleRejectB2BContract = (contractId: string, reason?: string) => {
    const updatedContracts = contracts.map((c: any) => {
      if (String(c.id) === String(contractId)) {
    ...
    if (rawV2) {
      try {
        ...
        const updatedV2 = parsedV2.map((c: any) => {
          if (String(c.id) === String(contractId)) {
            const up = { ...c, status: 'Đã từ chối', rejectReason: reason || 'Nội dung hợp đồng không đáp ứng quy chuẩn hệ thống.' };
            updatedSelected = up;
            return up;
          }
          return c;
    ...
    if (!updatedSelected) {
      updatedSelected = { ...selectedB2BContract, status: 'Đã từ chối', rejectReason: reason || 'Nội dung hợp đồng không đáp ứng quy chuẩn hệ thống.' };
    }
  ...
  const handleNegotiateB2BContract = (contractId: string, notes?: string) => {
    const updatedContracts = contracts.map((c: any) => {
      if (String(c.id) === String(contractId)) {
    ...
    if (rawV2) {
      try {
        ...
        const updatedV2 = parsedV2.map((c: any) => {
          if (String(c.id) === String(contractId)) {
            const up = { ...c, status: 'Thương thảo', negotiationNotes: notes || '' };
            updatedSelected = up;
            return up;
          }
          return c;
    ...
    if (!updatedSelected) {
      updatedSelected = { ...selectedB2BContract, status: 'Thương thảo', negotiationNotes: notes || '' };
    }
                    ...
                    <div>
                      ...
                      <span className="text-2xl font-black text-slate-800">
                        {contracts.filter((c: any) => 
                          ['Chờ duyệt', 'pending_super_admin', 'pending_coop', 'Chờ HTX duyệt', 'Chờ ký duyệt', 'coop_signed_pending_super', 'Thương thảo', 'Đang thương thảo'].includes(c.status) &&
                          c.type !== 'enterprise_htx' &&
                          c.type !== 'b2b'
                        ).length}
                      </span>
                    </div>
                ...
                <div className="space-y-8 animate-in fade-in duration-200">
                  ...
                  <div className="grid lg:grid-cols-2 gap-8">
                    ...
                    <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm relative overflow-hidden">
                      ...
                      <div className="flex items-center justify-between mb-8 relative z-10">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-500 font-medium font-bold">Hợp đồng liên kết với Liên hiệp HTX & Trung ương</p>
                          </div>
                        </div>
                        <button 
                      ...
                      <div className="space-y-4 relative z-10">
                        {filteredContracts.filter((c: any) => c.contractNo?.includes('HDLKTMHH')).length === 0 ? (
                          <div className="flex flex-col items-center justify-center py-12 px-4 bg-slate-50/50 rounded-3xl border border-dashed border-slate-200">
                            ...
                            <p className="text-sm font-bold text-slate-400">Chưa có hợp đồng nội bộ</p>
                          </div>
                        ) : (
                          filteredContracts.filter((c: any) => c.contractNo?.includes('HDLKTMHH')).map((item: any, i: number) => (
                            <div 
                              key={i} 
                              onClick={() => handleViewB2BContract(item)}
                            ...
                            >
                              ...
                              <div className="flex items-center gap-3">
                                <span className={`px-2 py-1 rounded-[8px] text-[9px] font-black uppercase tracking-widest ${
                                  ...
                                  {item.status === 'signed' && item.type !== 'individual' ? 'Đang thực hiện' : (item.status === 'pending_super_admin' ? 'Chờ duyệt' : item.status)}
                                </span>
                    ...
                    <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm relative overflow-hidden">
                      ...
                      <div className="space-y-4 relative z-10">
                        {filteredContracts.filter((c: any) => c.contractNo?.includes('HĐ-XV') || c.contractNo?.includes('HĐHTSXNN')).length === 0 ? (
                          <div className="flex flex-col items-center justify-center py-12 px-4 bg-slate-50/50 rounded-3xl border border-dashed border-slate-200">
                            <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center shadow-sm mb-4">
                              <FileSignature className="text-slate-300" size={24} />
                            </div>
                            <p className="text-sm font-bold text-slate-400">Chưa có hợp đồng xã viên</p>
                          </div>
                        ) : (
                          filteredContracts.filter((c: any) => c.contractNo?.includes('HĐ-XV') || c.contractNo?.includes('HĐHTSXNN')).map((item, i) => (
                            <div 
                              key={i} 
          ...
          >
            <div className="border-b border-slate-100 pb-5 mb-6 shrink-0">
              ...
              <div className="text-sm font-bold text-slate-500">
                  Số lượng hợp đồng: {filteredContracts.filter((c: any) => (c.contractNo?.includes('HĐ-XV') || c.contractNo?.includes('HĐHTSXNN')) && (c.party.toLowerCase().includes(individualContractsSearch.toLowerCase()) || c.cropName.toLowerCase().includes(individualContractsSearch.toLowerCase()))).length}
              </div>
            </div>
            <div className="overflow-y-auto space-y-4">
                ...
                />
              {filteredContracts.filter((c: any) => (c.contractNo?.includes('HĐ-XV') || c.contractNo?.includes('HĐHTSXNN')) && (c.party.toLowerCase().includes(individualContractsSearch.toLowerCase()) || c.cropName.toLowerCase().includes(individualContractsSearch.toLowerCase()))).map((item: any, i: number) => (
                <div 
                  key={i} 
                  onClick={() => {
      ...
      {isViewAllB2BContractsModalOpen && (
        <div className="fixed inset-0 z-[190] flex items-center justify-center p-4">
          ...
          >
            <div className="border-b border-slate-100 pb-5 mb-6 shrink-0">
              <div className="flex items-center justify-between mb-2">
                  <h3 className="text-2xl font-black text-forest">Hợp đồng Nội bộ</h3>
                  <button 
              ...
              <div className="text-sm font-bold text-slate-500">
                  Số lượng hợp đồng: {filteredContracts.filter((c: any) => c.contractNo?.includes('HDLKTMHH') && (c.party.toLowerCase().includes(b2bContractsSearch.toLowerCase()) || c.qty.toString().toLowerCase().includes(b2bContractsSearch.toLowerCase()))).length}
              </div>
            </div>
            <div className="overflow-y-auto space-y-4">
                <input 
                    type="text" 
                    placeholder="Tìm kiếm theo tên, sản lượng hoặc mã số..." 
                    value={b2bContractsSearch}
                    onChange={(e) => setB2BContractsSearch(e.target.value)}
                    className="w-full p-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm"
                />
               {filteredContracts.filter((c: any) => c.contractNo?.includes('HDLKTMHH') && (c.party.toLowerCase().includes(b2bContractsSearch.toLowerCase()) || (c.qty || '').toString().toLowerCase().includes(b2bContractsSearch.toLowerCase()))).map((item: any, i: number) => (
                <div 
                  key={i} 
                  onClick={() => {
                ...
                >
                   <div className="flex flex-col gap-1">
                     <span className="font-bold text-slate-800">{item.party}</span>
                     <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">{item.cropName || 'Nông sản'} • {item.qty}</span>
                   </div>
                   <span className={`px-2 py-1 rounded-[8px] text-[9px] font-black uppercase ${
                     ...
                     item.status === 'Đã từ chối' || item.status === 'rejected' || item.status === 'denied' || item.status === 'rejected_super_admin' || item.status === 'Hợp đồng đã bị từ chối' ? 'bg-rose-50 text-rose-600' :
                     item.status === 'Thương thảo' || item.status === 'Đang thương thảo' || item.status === 'negotiating' ? 'bg-purple-50 text-purple-600' :
                     item.status === 'Chờ duyệt' || item.status === 'pending_super_admin' || item.status === 'pending_coop' || item.status === 'Chờ HTX duyệt' || item.status === 'Chờ DN ký' || item.status === 'Chờ ký duyệt' || item.status === 'Chờ nông dân ký' || item.status === 'awaiting_farmer_signature' ? 'bg-amber-50 text-amber-600' :
                     'bg-slate-100 text-slate-500'
                   }`}>{item.status === 'signed' && item.type !== 'individual' ? 'Đang thực hiện' : (item.status === 'pending_super_admin' || item.status === 'pending_coop' || item.status === 'Chờ HTX duyệt' ? 'Chờ duyệt' : item.status)}</span>
                </div>
              ))}
            </div>
          </motion.div>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="flex-1 flex flex-col min-w-0 max-h-[80vh]">
              ...
              {/* Instant Search bar & Crop filter inside panel */}
              <div className="flex flex-col sm:flex-row gap-3 mb-6 shrink-0">
                <div className="flex-[3] flex items-center gap-3 bg-slate-50 px-4 py-3 rounded-2xl border border-slate-100 shadow-inner">
                ...
                <div className="flex-[2] relative">
                  <select
                    value={harvestFilterCrop}
                    onChange={(e) => setHarvestFilterCrop(e.target.value)}
                    className="w-full bg-slate-50 border border-slate-100 pl-4 pr-10 py-3 text-xs font-bold text-forest rounded-2xl outline-none shadow-sm cursor-pointer appearance-none"
                  >
              ...
              <div className="flex-1 overflow-y-auto space-y-3 min-h-0 pr-1 select-none">
                {filteredFarmersForHarvest.length === 0 ? (
                  <div className="text-center py-8 text-slate-400 text-xs font-bold uppercase tracking-wider">
                    Không tìm thấy nông dân hoặc nông sản phù hợp
                  </div>
                ) : (
                  filteredFarmersForHarvest.map((farmer, idx) => {
                    ...
                    return (
                      <div 
                        ...
                        onClick={() => {
                          ...
                          const farmerData = activeFarmers.find(f => f.name === farmerName);
                          // Determine auto-selected crop
                          const matchedCrop = (farmerData?.crops || []).find((c: any) => c.name === harvestFilterCrop) || farmerData?.crops[0];
                          
                          setNewHarvest({
                            ...newHarvest,
                            farmer: farmerName,
                            product: matchedCrop?.name || '',
                            area: matchedCrop?.area || '',
                            batch: ''
                          });
                        }}
                      ...
                      >
                        ...
                        {/* Farmer Registered Crops Detail Box */}
                        {farmer.crops && farmer.crops.length > 0 ? (
                          <div className="border-t border-slate-100 pt-2.5 space-y-1.5 w-full">
                            <p className="text-[9px] font-black uppercase tracking-widest text-emerald-800">Cây trồng & Dự kiến thu hoạch:</p>
                            <div className="grid grid-cols-1 gap-1.5 w-full font-sans">
                              {farmer.crops.map((crop: any, cidx: number) => {
                                const isCurrentFilteredCrop = harvestFilterCrop !== 'all' && crop.name === harvestFilterCrop;
                                return (
                                  <div 
                                    key={cidx} 
                                    className={`px-3 py-2 rounded-xl text-xs flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 border transition-all ${
                                      isCurrentFilteredCrop 
                                        ? 'bg-emerald-50 border-emerald-200' 
                                        : 'bg-slate-50 border-slate-100/50 group-hover:border-slate-200/50'
                                    }`}
                                  >
                                    <div className="min-w-0 flex items-center gap-1.5">
                                      <div className={`w-1.5 h-1.5 rounded-full shrink-0 ${isCurrentFilteredCrop ? 'bg-emerald-500 animate-pulse' : 'bg-forest/40'}`} />
                                      <span className="font-bold text-slate-800 truncate">{crop.name}</span>
                                    </div>
                                    <div className="flex flex-wrap items-center gap-x-2 gap-y-1 shrink-0 text-[10px] font-medium text-slate-500">
                                      <div>
                                        <span className="text-slate-400 font-semibold">Thu hoạch:</span>{' '}
                                        <span className="font-extrabold text-forest">{crop.expectedHarvestDate || 'Chưa cập nhật'}</span>
                                      </div>
                                      <div className="w-px h-2.5 bg-slate-200 hidden sm:block" />
                                      <div>
                                        <span className="text-slate-400 font-semibold">Sản lượng:</span>{' '}
                                        <span className="font-extrabold text-blue-600">{crop.expectedYield || '--'} {crop.yieldUnit || 'tấn'}</span>
                                      </div>
                                    </div>
                                  </div>
                                );
                          ...
                          <div className="border-t border-slate-100 pt-2 space-y-1.5 w-full text-center">
                            <p className="text-[10px] text-slate-400 font-bold italic">Chưa cập nhật thông tin cây trồng</p>
                          </div>
                        )}
                      </div>
                    );
      ...
      {selectedB2BContract && (
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-md overflow-y-auto">
          ...
          >
            ...
            {/* Rejection / Negotiation Notes */}
            {['Hợp đồng đã bị từ chối', 'rejected_super_admin', 'rejected'].includes(selectedB2BContract.status) && selectedB2BContract.rejectReason && signingStep === 'idle' && (
              <div className="bg-rose-50 border border-rose-200 rounded-2xl p-5 font-sans text-rose-800 text-sm shadow-sm flex items-start gap-4">
                <div className="w-10 h-10 rounded-full bg-rose-100 flex items-center justify-center text-rose-600 shrink-0 mt-0.5">
                  <XCircle size={20} />
                </div>
                <div>
                  <h4 className="font-black uppercase tracking-wider mb-1 text-rose-900">Hợp đồng bị từ chối</h4>
                  <p className="font-medium opacity-90 leading-relaxed">{selectedB2BContract.rejectReason}</p>
                </div>
              </div>
            )}
            
            {['Thương thảo', 'negotiating'].includes(selectedB2BContract.status) && selectedB2BContract.negotiationNotes && signingStep === 'idle' && (
              <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 font-sans text-amber-800 text-sm shadow-sm flex items-start gap-4">
                <div className="w-10 h-10 rounded-full bg-amber-100 flex items-center justify-center text-amber-600 shrink-0 mt-0.5">
                  <Clock size={20} />
                </div>
                <div>
                  <h4 className="font-black uppercase tracking-wider mb-1 text-amber-900">Yêu cầu thương thảo điều chỉnh hợp đồng</h4>
                  <p className="font-medium opacity-90 leading-relaxed">{selectedB2BContract.negotiationNotes}</p>
                </div>
              </div>
            )}
            ...
            {signingStep !== 'idle' ? (
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-[2rem] p-8 min-h-[400px] flex flex-col items-center justify-center text-center">
                <div className="max-w-md w-full space-y-6">
                  ...
                  <div className="text-center font-sans">
                    ...
                    <p className="text-sm text-slate-500 mt-2 max-w-sm mx-auto">Vui lòng nhập lý do từ chối phê duyệt hợp đồng này để gửi phản hồi cho đối tác.</p>
                  </div>
                  <div className="space-y-4 font-sans">
                    <textarea 
                      placeholder="Nhập lý do từ chối (Ví dụ: Sản lượng không đúng đợt thu hoạch...)"
                      rows={4}
                  ...
                  <div className="flex gap-4 pt-2 font-sans">
                    <button
                      ...
                      className="w-1/2 py-4 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-2xl font-black text-xs uppercase transition-all"
                    >
                      Bỏ qua
                    </button>
                    <button
                      ...
                      className="w-1/2 py-4 bg-rose-600 hover:bg-rose-700 text-white rounded-2xl font-black text-xs uppercase shadow-md shadow-rose-600/20 transition-all"
                    >
                      Xác nhận từ chối
                    </button>
                  </div>
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-[2rem] p-8 min-h-[400px] flex flex-col items-center justify-center text-center">
                <div className="max-w-md w-full space-y-6">
                  ...
                  <div className="text-center font-sans">
                    ...
                    <p className="text-sm text-slate-500 mt-2 max-w-sm mx-auto">Vui lòng nhập nội dung đề xuất thương thảo/điều chỉnh để phản hồi cho đối tác.</p>
                  </div>
                  <div className="space-y-4 font-sans">
                    <textarea 
                      placeholder="Nhập nội dung đề xuất thương thảo (Ví dụ: Yêu cầu điều chỉnh đơn giá cà phê thành 135.000 VNĐ/kg...)"
                      rows={4}
                  ...
                  <div className="flex gap-4 pt-2 font-sans">
                    ...
                    <button
                      ...
                      className="w-1/2 py-4 bg-amber-500 hover:bg-amber-600 text-white rounded-2xl font-black text-xs uppercase shadow-md shadow-amber-500/20 transition-all"
                    >
                      Gửi yêu cầu
                    </button>
                  </div>
              ...
              <div className="bg-slate-50 border border-slate-200/80 rounded-[2rem] p-6 md:p-12 text-[#1c1c1c] font-serif leading-relaxed text-sm space-y-8 max-h-[68vh] overflow-y-auto scrollbar-thin shadow-inner select-text relative">
                ...
                <div className="text-center font-serif py-3 space-y-1">
                  ...
                  <p className="text-xs font-sans tracking-wide">Số: {cleanContractNo(selectedB2BContract.contractNo, selectedB2BContract.id)}</p>
                </div>
              ...
              >
               ...
               {signingStep === 'idle' && (
                !(selectedB2BContract.status === 'signed' || 
                  selectedB2BContract.status === 'completed' || 
                  selectedB2BContract.status === 'active' || 
                  selectedB2BContract.status === 'Đang thực hiện' || 
                  selectedB2BContract.status === 'Đã hoàn tất' || 
                  selectedB2BContract.status === 'Hoàn tất' || 
                  selectedB2BContract.status === 'Thương thảo' || 
                  selectedB2BContract.status === 'Đang thương thảo' || 
                  selectedB2BContract.status === 'Chờ DN ký' || 
                  selectedB2BContract.status === 'Chờ nông dân ký' || 
                  selectedB2BContract.status === 'Nông dân đã ký' || 
                  selectedB2BContract.status === 'Hợp đồng đã bị từ chối' || 
                  selectedB2BContract.status === 'rejected_super_admin' || 
                  selectedB2BContract.status === 'rejected') ? (
                  <>
                    <button
                      type="button"
                      onClick={() => setIsRejectingInline(true)}
                      className="px-6 py-3.5 bg-slate-100 hover:bg-rose-50 text-slate-500 hover:text-rose-600 rounded-2xl font-black text-xs uppercase tracking-widest transition-all cursor-pointer flex items-center gap-1.5"
                    >
                    ...
                    {selectedB2BContract.status !== 'Hợp đồng đã bị từ chối' && (
                      <>
                        <button
                          ...
                          className="px-6 py-3.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-2xl font-black text-xs uppercase tracking-widest transition-all cursor-pointer flex items-center gap-1.5"
                        >
                ...
                ) : (selectedB2BContract.status === 'signed' || selectedB2BContract.status === 'completed' || selectedB2BContract.status === 'active' || selectedB2BContract.status === 'Đang thực hiện' || selectedB2BContract.status === 'Đã hoàn tất' || selectedB2BContract.status === 'Hoàn tất' || selectedB2BContract.status === 'Nông dân đã ký') ? (
                  <button
                    type="button"
                ...
                ) : (selectedB2BContract.status === 'Hợp đồng đã bị từ chối' || selectedB2BContract.status === 'rejected_super_admin' || selectedB2BContract.status === 'rejected') ? (
                  <button
                    type="button"
                    onClick={() => {
