  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        let found = contracts.find((c: any) => String(c.id) === String(contractId));
        
        // Fallback or secondary check in local storage if not in API (migration phase)
        if (!found) {
          const updateFromSource = (raw: string | null) => {
            if (!raw) return;
            try {
              const arr = JSON.parse(raw);
              return arr.find((c: any) => String(c.id) === String(contractId));
            } catch (e) {
              return null;
            }
          };
          found = updateFromSource(localStorage.getItem('vietagri_contracts_v2')) || 
        ...
        const found = updateFromSource(localStorage.getItem('vietagri_contracts_v2')) || 
                      updateFromSource(localStorage.getItem('vietagri_contracts_v3'));
  ...
  const completeSignature = async () => {
    ...
    try {
      ...
      const updatedData = {
        ...
        farmerSignatureCode: sigCode
      };

      // 1. Update via API
      await contractService.updateContract(contract.id, updatedData);
      ...
      // Also update other local storage trackers for dashboard consistency
      ...
      alert("Lỗi khi ký hợp đồng! Vui lòng thử lại.");
    }
  };
