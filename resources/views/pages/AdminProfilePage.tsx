    ...
    useEffect(() => {
        ...
        const fetchAdmin = async () => {
            try {
                ...
                if (user) {
                    const htx = COOPERATIVES.find(c => c.id === user.cooperativeId);
                    setAdmin({
                        ...
                        htxName: htx ? htx.name : (user.htxName || 'HTX Cà phê Cầu Đất'),
                        cooperativeId: user.cooperativeId || 'HTX-001',
                        address: user.address || "Tòa nhà Agri-Digital, Thành phố Đà Lạt",
                        avatar: user.avatar
                    });
                }
            } catch (err) {
