...
export default function EnterpriseProfilePage() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      ...
      try {
        ...
        const phone = user?.phone || localStorage.getItem('userPhone') || '0901234570';
        const name = user?.name || localStorage.getItem('userName') || 'Doanh Nghiệp Đối Tác';
        const email = user?.email || localStorage.getItem('userEmail') || 'customer@vietagri.vn';
        ...
        setUserData({
          ...
          name,
          email,
          taxCode: user?.taxCode || '0312345678',
          address: user?.address || 'Hà Nội',
          website: user?.website || 'www.vietagri-partner.vn'
        });
      } catch (err) {
        console.error('Fetch user error', err);
        const phone = localStorage.getItem('userPhone') || '0901234570';
        const name = localStorage.getItem('userName') || 'Doanh Nghiệp Đối Tác';
        const email = localStorage.getItem('userEmail') || 'customer@vietagri.vn';
        setUserData({
          phone,
          name,
          email,
          taxCode: '0312345678',
          address: 'Hà Nội',
          website: 'www.vietagri-partner.vn'
        });
      }
    };
    fetchUser();
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      ...
      setUpdateMessage('Cập nhật hồ sơ doanh nghiệp thành công!');
              ...
              <div className="relative h-full flex flex-col md:flex-row items-center md:items-end p-8 md:p-12 gap-8">
                ...
                <div className="flex-1 text-center md:text-left mb-4">
                  <div className="flex flex-col md:flex-row md:items-center gap-3 mb-2">
                    <input 
                      type="text"
                      className="text-3xl md:text-5xl font-black text-white italic tracking-tighter bg-transparent border-none focus:ring-0 w-full"
                      value={userData.name}
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            >
              <div className="flex items-center justify-between mb-10 pb-6 border-b border-slate-50">
                ...
                <div className="flex items-center gap-4">
                  {updateMessage && <span className="text-emerald-500 text-[10px] font-bold uppercase tracking-widest">{updateMessage}</span>}
                  <button 
                    onClick={handleUpdateProfile}
                    disabled={isUpdating}
                    className="px-6 py-3 bg-[#004d40] text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:opacity-90 transition-all disabled:opacity-50"
                  >
              ...
              <div className="grid md:grid-cols-2 gap-x-12 gap-y-10">
                {[
                  { field: 'taxCode', label: "Mã số thuế", value: userData.taxCode || '0123456789', icon: <Fingerprint size={18} /> },
                  { field: 'email', label: "Email liên hệ", value: userData.email || 'info@enterprise.vn', icon: <Mail size={18} /> },
                  { field: 'phone', label: "Số điện thoại", value: userData.phone, icon: <Phone size={18} /> },
                  { field: 'website', label: "Website", value: userData.website || "www.vietagri-partner.vn", icon: <Globe size={18} /> }
                ].map((item, i) => (
                  <div key={i} className="group">
                    ...
                    <div className="flex items-center gap-3 text-slate-700">
                      ...
                      <input 
                        className="text-base font-black bg-transparent border-b border-transparent focus:border-emerald-500 focus:ring-0 p-0 w-full"
                        value={item.value}
